...
let embeddingsData;
try {
    embeddingsData = await import('../data/embeddings-gemini.json', { assert: { type: 'json' } });
    embeddingsData = embeddingsData.default;
} catch (error) {
    console.warn('Gemini embeddings not found, falling back to OpenAI embeddings');
    try {
        embeddingsData = await import('../data/embeddings.json', { assert: { type: 'json' } });
        embeddingsData = embeddingsData.default;
    } catch (fallbackError) {
...
export function getGeminiVectorStore() {

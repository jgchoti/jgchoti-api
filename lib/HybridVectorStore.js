...
async function loadEmbeddings() {
    ...
    try {
        const module = await import('../data/embeddings-gemini.json', { assert: { type: 'json' } });
        embeddingsData = module.default;
        return embeddingsData;
    } catch (error) {
        console.warn('Gemini embeddings not found, falling back to OpenAI embeddings');
        try {
            const module = await import('../data/embeddings.json', { assert: { type: 'json' } });
            embeddingsData = module.default;
            return embeddingsData;
        } catch (fallbackError) {
